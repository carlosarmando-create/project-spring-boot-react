      ...
      <section className="mt-6 grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="glass-card overflow-hidden">
          <div className="relative min-h-95">
            <Image
              src={resolveAssetUrl(plant.imageUrl)}
              alt={plant.name}
              fill
        ...
        <article className="glass-card grid gap-5 p-8">
          ...
          <div className="grid gap-3 rounded-3xl bg-white/65 p-5 text-sm text-(--muted)">
            <p>
              ...
              {plant.botanicalName || "No especificado"}
            </p>
            <p>
              <span className="font-semibold text-(--foreground)">
                Presentacion:
              </span>{" "}
              {plant.sizeLabel || "Tamaño estándar"}
            </p>
            <p>
              <span className="font-semibold text-(--foreground)">
                Stock disponible:
              </span>{" "}
              {plant.stock} unidades
            </p>
            <p>
              <span className="font-semibold text-(--foreground)">
                Precio referencial:
              </span>{" "}
              S/ {Number(plant.price).toFixed(2)}
            </p>
          </div>
